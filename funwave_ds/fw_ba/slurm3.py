...
def write_slurm_script(work_dir,run_name,matrix,all_slurm_flags,script_body=None):
    ...
    batch_dir = os.path.join(work_dir, 'runs',run_name, 'batch_scripts',matrix)
...
def write_slurm_script2(work_dir,run_name,matrix,all_slurm_flags,script_body=None):
    # Make folder if it doesn't exist for batch scripts
...
def make_log_folders(work_dir,run_name,matrix,all_slurm_flags):
    ...
    if all_slurm_flags.get('array') is None:
        ...
        out_err_dir = os.path.join(work_dir, 'runs',run_name, 'logs',matrix,job_name)
    ...
    # Separate dir for outputs and errors if an array
        ...
        job_name = all_slurm_flags['job-name']
        out_dir = os.path.join(work_dir, 'runs',run_name, 'logs',matrix,job_name,'out')
        err_dir = os.path.join(work_dir, 'runs',run_name, 'logs',matrix,job_name,'err')
...
def make_log_folders2(work_dir,run_name,matrix,all_slurm_flags):
    # Just one folder if no array
    if all_slurm_flags.get('array') is None:
        job_name = all_slurm_flags['job-name']
...
class SlurmPipeline:
    def __init__(self, slurm_vars, work_dir, run_name, matrix, env_name):
    ...
    def add_job(self, dep_flags, script_content_func, **kwargs):
        ...
        all_slurm_flags = make_log_folders(self.work_dir, self.run_name,self.matrix,all_slurm_flags)
...
class SlurmPipeline2:
    def __init__(self,slurm_vars = None):
        # Dictionary of defauly slurm variables
        self.slurm_vars = slurm_vars
    ...
    def add_job(self, dep_flags, script_content_func, **kwargs):
        ...
        all_slurm_flags = {**self.slurm_vars, **slurm_edit, **dep_flags}
        # Make log folders, set in slurm flags
        all_slurm_flags = make_log_folders2(self.work_dir, self.fw_model,self.run_name,all_slurm_flags)

        ## Body of Script
        script_body = script_content_func(**kwargs)
        
        # Write the slurm script
        script = write_slurm_script2(self.work_dir,
                                        self.fw_model,
                                        self.run_name,
                                        all_slurm_flags,
                                        script_body)
